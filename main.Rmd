...
# Simulation Study {#simulation-study}
In this simulation study, we will examine the Friedman procedure for two sample testing. The goal of the test is to check whether two probability densities are equal, $p(x) = p_0(x)$, by training a **binary classifier** on the dataset containing samples from two classes, and calculating **test statistics** on the scores that the classifier assigns to the samples to calculate a **p-value**. We will use the **permutation** variation, which compares the chosen test statistic calculated on the **original dataset** with the $1 - \alpha$ quantile of a set of $p$ test statistics that were measured on a dataset where the response variables have been permuted. In order to test $H_{0}$ we need to know the null distribution $H_{0}(t)$, but since we are using the same data for training and scoring, the null distribution is not valid, so we permute to make it valid.

For this study, we will use **Logistic Regression** classifier and the **Kolmogorov-Smirnov** test statistic.
...
produce.test.statistics.par = function(df, p, model = "lg") {
  #' In parallel, produces a set of p test statistics by permuting the labels of the df dataframe p times,
  #' training a glm classifier on each permuted set, and doing a test statistic on the output scores.
  #' 
...
## Friedman Test Function {#friedman-test-function}
We are running the Friedman test with permuting of the labels in the dataset. The `friedman.test.permute.par` function first calculates the **Kolmogorov-Smirnov** statistic on the original data, then permutes the labels $p$ times, calculating the **Kolmogorov-Smirnov** statistic at each permutation. The permutations are done in parallel. Finally, the p-value is calculated by getting the proportion of permuted KS statistics that are greater than or equal to the original statistic. $$p = \frac{1}{N} \sum\limits^{N}_{i=1} \mathbb{I}\left(T_i \ge T \right)$$
...
## Simulation Loop {#simulation-loop}
The `simulate` function runs $M$ number of simulations to find the **p-value** of the **Kolmogorov-Smirnov** two sample test, using permutation of the response variables. In each simulation, we:

1. Create a **k-variate** dataset, under $H_0$ or $H_1$.
1. Run the [Friedman test](#friedman-test-function).
1. Save the **p-value**.
...
```{r H1 simulation, echo=FALSE, warning=FALSE, out.width="100%"}
...
```{r H0 simulation, echo=FALSE, warning=FALSE, out.width="100%"}
totalCores = detectCores()
...
```{r experiment new params simulation, warning=FALSE, eval=FALSE}
k.values = seq(3, 7)
...
The plot below shows the **power** of the test, i.e. observing a **true positive** (complement of false negative) as a function of the **number of features** $k$, and the **number of samples**. Again, each circle is the **average p-value** of $M$ simulations. Here we can see that the probability of correctly rejecting the null hypothesis when it is false increases with the number of features and somewhat with the number of samples. For $k=3$, the probability of **true discovery** is lower than for $k=7$, indicating a positive relationship between the two variables. Additionally as the number of features grows the number of samples becomes less important in determining the probability of **true positive** as the density of the circles shows. This could be explained by the fact that as the number of dimensions increase, the amount of useful information encoded in them which helps the Logistic Regression model separate the classes also increases.
...
Assuming $\alpha = 0.05$ there are not enough evidences for rejecting the null hypothesis; however if we choose a greater $\alpha$ we also increase the risk of Type I error probability and we may wrongly reject $H_{0}$ increasing the power of the statistical test. Considering the p-value below, with an $\alpha = 0.15$ we could reject $H_{0}$.
```{r echo=FALSE}
fmri.p$p
```
