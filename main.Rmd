...
Assuming $\alpha = 0.05$ there are enough evidences for rejecting the null hypothesis; however if we choose a greater $\alpha$ we also increase the risk of Type I error probability and we may wrongly reject $H_{0}$ increasing the power of the statistical test. Considering the p-value below, with an $\alpha = 0.15$ we could reject $H_{0}$.
```{r echo=FALSE}
fmri.p$p
```
