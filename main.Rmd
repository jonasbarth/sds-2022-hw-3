...
for (i in 1:nrow(params.df)) {
  ...
  params.df$size[i] = sum(simulate(M, n0, n1, row$k, create_df, H0=TRUE) < 0.05) / M
  params.df$power[i] = sum(simulate(M, n0, n1, row$k, create_df, H0=FALSE) < 0.05)/M
}
write.csv(params.df, 'params.csv', row.names = FALSE) 
```

```{r read params from disk, include=FALSE}
params.df = read.csv('params.csv')

k.values = sort(unique(params.df$k))
N.values = sort(unique(params.df$N))
prop.values = sort(unique(params.df$prop))
```

```{r}
#for (N in N.values) {
#  y = params.df$size[(params.df$N == N) & (params.df$prop == 0.5)]
#  lines(k.values, y, col='orange', type='l')
#}

params.df$N = as.character(params.df$N)
ggplot(data=params.df[params.df$prop == 0.2, ], aes(x=k, y=power, col=N, group=1)) +
  geom_line()+
  geom_point()
