...
f <- function(...) {
  ...
  lapply(l, function(el, label) print(label))
}
...
importance = theta$s99 %>% abs() 
top.10 = importance > quantile(importance, 0.95)
# set intercept to False
top.10[1] = FALSE

theta$feature_name = row.names.data.frame(theta)

theta[top.10, ] |>
  ggplot(aes(x = reorder(feature_name, abs(s99)), y = s99)) +
  geom_col() +
  coord_flip()
```
