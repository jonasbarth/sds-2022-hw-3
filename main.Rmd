...
The three dot operator works similarly to the `**kwargs` operator in python. It allows to enter any number of named parameters, which can then be parsed through.

Three dots operator demonstration:
```{r eval=FALSE}
f <- function(...) {
  l = list(...)
...
theta[top.10, ] |>
  ggplot(aes(
    x = reorder(feature_name, abs(s99)), 
    y = s99,
    fill = ifelse(sign(s99) == 1, 'TD', 'ASD'))) +
  geom_col() +
  coord_flip() +
  labs(
    x = '',
