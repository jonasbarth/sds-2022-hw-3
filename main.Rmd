...
```{r include=FALSE}
produce.test.statistics.par = function(df, p) {
  test.statistics <- foreach(i = 1:p, .combine=c, .packages="e1071", .export="permute.labels") %dopar% {
    permuted.df = permute.labels(df)
    
    model = svm(label ~ ., data=permuted.df, kernel="sigmoid", shrinking=FALSE, probability=TRUE)
    y_pred = predict(model, permuted.df)
  ...
  return(list(statistics=test.statistics, pred=y_pred))
}
```
...
train.predict = function(df) {
  ...
  model = svm(label ~ ., data=df, kernel="sigmoid", shrinking=FALSE, probability=TRUE)
...
friedman.test.permute.par = function(df, alpha = 0.05, p = 1e2) {
  ...
  test.statistics = produce.test.statistics.par(df, p)
...
simulate.par = function(M, n0, n1, k, create.data, H0) {
    ...
    results = foreach(i = 1:M, .combine=c, .packages = "e1071", .export = c("random.data", "random.distro", "friedman.test.permute", "train.predict", "produce.test.statistic", "permute.labels")) %dopar% {
...
simulate = function(M, n0, n1, k, create.data, H0) {
    ...
    for (i in 1:M) {
        ...
        p.value = friedman.test.permute.par(generated$df, alpha = alpha, p = n0 + n1)
...
results.H1[2]
H1.p.values = sapply(results.H1, function(res) res$p)

h1.hist = hist(results.H1$statistics, 
     breaks = 20,
...
fmri.p = friedman.test.permute.par(ts.feat, alpha = 0.05, p = 1e3)
