...
simulate = function(M, n0, n1, k, create.data) {
    ...
    for (i in 1:M) {
        ...
        #model = glm(formula=label ~ ., data=df, family=binomial)
        X = model.matrix(label ~ ., df)[,-1]
        y = df[,ncol(df)]
        
        model = glmnet(X, y, alpha = 1, family = "binomial", lambda = 0.2)
        ...
        return(df)
        hist(y_pred_class_0, col =rgb(1,0,0,0.2), probability = TRUE)
        hist(y_pred_class_1, add = T, col = rgb(0,1,0,0.2), probability = TRUE)
...
```{r}
results = simulate(1, 100, 150, k, create_df)

hist(results$y_pred)
summary(results$y_pred)

y_pred_class_1 = results[results$label == 1,"y_pred"]
y_pred_class_0 = results[results$label == 0,"y_pred"]

hist(y_pred_class_1, probability = TRUE, col = rgb(0,0,1,0.5))
hist(y_pred_class_0, add = TRUE, col = rgb(1,0,0,0.5), probability = TRUE)

ks.test(y_pred_class_0, y_pred_class_1)
